function grka_warmstart(path_to_input, filename, verbose_ws, sortmethod, upper_bound, lower_bound, tugs, K, T, S, Q, P_s, P_q, break_length, x, y, wqq, wqv, wvq, wqb, sb, u)
    ...
    for i in (1): (upper_bound - break_length) # at the beggining assign each action to being idle ('WQQ')
    ...
    add_break(tugs_array, lower_bound, break_tugs_array, break_length)
    ### ASSIGN MODEL'S VARIABLES WITH WARMSTART OUTPUT ###
    assign_variables(break_tugs_array, x, y, wqq, wqv, wvq, wqb, sb)
    objective = t+break_length
    set_start_value(u, objective)

    ### PRINT THE RESULTS ###
    if verbose_ws == true
        print_warmstart(tugs, upper_bound, x, y, wvq, wqv, wqq, wqb, K, T, S, filename)
...
function add_break(tugs_array, lower_bound, break_tugs_array, break_length)
    break_start = floor(lower_bound/2)

    for (tug_index, tug) in enumerate(tugs_array)
        for (index, action) in enumerate(tug)
            if index == break_start
    ...
    for (tug_index, tug) in enumerate(tugs_array)
        for (action_index, action) in enumerate(tug)
            ...
            elseif  action == "WQB s"
                ...
                set_start_value(sb[action_index, tug_index], 1)
            elseif  action == "WQB"
                set_start_value(wqb[action_index, tug_index], 1)
...
function print_warmstart(tugs, upper_bound, x, y, wvq, wqv, wqq, wqb, K, T, S, filename)
    header = ["Time"]
    for k in K
