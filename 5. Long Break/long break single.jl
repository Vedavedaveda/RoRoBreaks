function single_warmstart(path_to_input, filename, verbose_ws, upper_bound, tugs, K, T, S, P_s, P_q, b, x, y, wvq, wqv, wqq, wqb, sb, u)
    ...
    ### BREAK ###
    t = make_break(qk, halftugs, b, wqb, sb, t)
    ...
    if verbose_ws == true
        print_single_warmstart(tugs, upper_bound, x, y, wvq, wqv, wqq, wqb, K, T, S, filename)
...
function make_break(qk, halftugs, b, wqb, sb, t)
    for i in 1:halftugs # Give half of the tugs breaks
        k = pop!(qk)                            
        pushfirst!(qk,k)
        set_start_value(sb[t,k], 1)
        for i in 0:(b-1)
...
function print_single_warmstart(tugs, upper_bound, x, y, wvq, wqv, wqq, wqb, K, T, S, filename)
    header = ["Time"]
    for k in K
