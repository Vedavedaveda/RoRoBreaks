...
using ProgressMeter, Dates, LightGraphs, PrettyTables, CSV
...
function dualcycling(; verbose = true, verbose_ws = true, rule = "mp", instance = "0", warmstart = "grka", sortmethod = "kids", 
    ...
    if warmstart == "grka"
        filename = string("simpleBreak_", rule, instance, "_", warmstart, "_", sortmethod)
    else
        filename = string("simpleBreak_", rule, instance, "_", warmstart)
    end
...
#################################### Objective & Constraints ##########################################    
    ...
    @constraint(model, c20[t in T[bs:bs+bl-1]], wqq[t] == k)

#################################### Warm Start ##########################################    
   
    if warmstart == "single"
        include("2. Simple Break/simple break single.jl")
        ws_objective = single_warmstart(path_to_input, filename, verbose_ws, upper_bound, tugs, T, S, P_s, P_q, bs, bl, x, y, wqq, wqv, wvq, u)
    end

    if warmstart == "grka"
        include("2. Simple Break/simple break grka.jl")
...
#################################### Optimize ##########################################    
    ...
    save_results(model, rule , instance, warmstart, sortmethod, ws_objective, "simple break", "results.csv")
    if verbose == true
