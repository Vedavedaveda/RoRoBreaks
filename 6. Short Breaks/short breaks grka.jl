function grka_warmstart(path_to_input, filename, verbose_ws, sortmethod, upper_bound, tugs, K, T, S, Q, P_s, P_q, break_length, ru, rl, N, x, y, wqq, wqv, wvq, wqb, sb, u)
    ...
    add_break(tugs_array, T, break_tugs_array, break_length, rl, ru, N, t)
    ...
    assign_variables(break_tugs_array, x, y, wqq, wqv, wvq, wqb, sb)
    ...
    ### PRINT THE RESULTS ###
    if verbose_ws == true
        print_warmstart(filename, tugs, upper_bound, x, y, wvq, wqv, wqq, wqb, K, T, S, N)
...
function add_break(tugs_array, T, break_tugs_array, break_length, rl, ru, N, t)
    repeat_break = floor((rl+ru)/2) 
    first_start = repeat_break 
    break_start_indices = []
    for i in N
        push!( break_start_indices, repeat_break*i - (i-1)*break_length)
    end


    #break_start_indices = first_start:repeat_break:length(T)
    total_break_num = length(N)
    for (tug_index, tug) in enumerate(tugs_array)
        break_num = 0
        for (index, action) in enumerate(tug)

            if index in break_start_indices
                ...
                if action == "WQQ" || action == "WQV" || split(action, " ")[1] == "load"
                    ...
                    push!(break_tugs_array[tug_index], "WQB s $break_num")
                    for i in 1: (break_length - 1)
                        push!(break_tugs_array[tug_index], "WQB $break_num")
                    end
                    push!(break_tugs_array[tug_index], action)
                else
                    push!(break_tugs_array[tug_index], action)
...
function assign_variables(tugs_array, x, y, wqq, wqv, wvq, wqb, sb)
    for (tug_index, tug) in enumerate(tugs_array)
        for (action_index, action) in enumerate(tug)
            ...
            elseif  split(action, " ")[1] == "WQB"
                if split(action, " ")[2] == "s"
                    break_num =parse(Int64,(split(action, " ")[3]))
                    set_start_value(sb[action_index, tug_index, break_num], 1)
                    set_start_value(wqb[action_index, tug_index, break_num], 1)
                else
                    break_num = parse(Int64,split(action, " ")[2])
                    set_start_value(wqb[action_index, tug_index, break_num], 1)
                end

            elseif split(action, " ")[1] == "load"
                slot_name = split(action, " ")[2]
                set_start_value(y[slot_name, action_index, tug_index], 1)
            elseif split(action, " ")[1] == "unload"
                slot_name = split(action, " ")[2]
...
function print_warmstart(filename, tugs, upper_bound, x, y, wvq, wqv, wqq, wqb, K, T, S, N)
    header = ["Time"]
    for k in K
    ...
    for t in 1:(upper_bound)
        ...
        for k in K
            ...
            elseif ((start_value(wqq[t,k]))==1)
                push!(row, "$k : IDLE")
            elseif (sum(start_value(wqb[t,k,n]) for n in N)==1)
                push!(row, "$k : BREAK")
            else
                for i in S
