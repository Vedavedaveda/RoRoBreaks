...
function dualcycling(; verbose = true, verbose_ws = true, rule = "mp", instance = "0", warmstart = "grka", sortmethod = "kids", 
    ...
    if warmstart == "grka"
        filename = string("tugID_", rule, instance, "_", warmstart, "_", sortmethod)
    else
        filename = string("tugID_", rule, instance, "_", warmstart)
    end
...
#################################### Decision Variables ##########################################   
    ...
    @constraint(model, c12[t in T, k in K], (sum(x[i,t,k] for i in S) + wvq[t,k] + sum(y[j,t,k] for j in Q) + wqv[t,k] + wqq[t,k]) == 1)
    ...
    # (18) Ensures that x[i,t] and y[j,t] are binary (not necessary since we defined them as such) (also ensure that bs is binary, and also all the rest (wqq, wqv, etc.)))
...
#################################### Warm Start ##########################################    
    ...
    if warmstart == "single"
        include("4. Tug ID/tug id single.jl")
        ws_objective = single_warmstart(path_to_input, filename, verbose_ws, tugs, K, upper_bound, T, S, P_s, P_q, x, y, wqq, wqv, wvq, u)
    end

    if warmstart == "grka"
        include("4. Tug ID/tug id grka.jl")
        ws_objective = grka_warmstart(path_to_input, filename, verbose_ws, sortmethod, upper_bound, tugs, K, T, S, Q, P_s, P_q, x, y, wqq, wqv, wvq, u)
    end
...
#################################### Save Results ##########################################    
    ...
    save_results(model, rule , instance, warmstart, sortmethod, ws_objective, "tug id", "results.csv")
    if verbose == true
        save_loading_plan(tugs, upper_bound, x, y, wvq, wqv, wqq, T, S, K, filename)
    end
...
function save_loading_plan(tugs, upper_bound, x, y, wvq, wqv, wqq, T, S, K, filename)
    header = ["Time"]
    for k in K
