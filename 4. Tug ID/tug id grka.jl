function grka_warmstart(path_to_input, filename, verbose_ws, sortmethod, upper_bound, tugs, K, T, S, Q, P_s, P_q, x, y, wqq, wqv, wvq, u)
    ...
    ### PRINT THE RESULTS ###
    if verbose_ws == true
        print_warmstart(tugs, upper_bound, x, y, wvq, wqv, wqq, K, T, S, filename)
...
function print_warmstart(tugs, upper_bound, x, y, wvq, wqv, wqq, K, T, S, filename)
    header = ["Time"]
    for k in K
