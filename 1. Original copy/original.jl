...
# verbose_ws = if true, warmstart is saved as .txt
# rule = precedence matrix rule: m, mp, ms, mps
# instance = from 0 to 10
# warmstart = grka, single, or none
# sortmethod = kids or top (only applicable to grka warmstart)
...
#using DataFrames, XLSX, Gurobi, JuMP, CSV, DataStructures, PrettyTables
import MathOptInterface

######################################## Dual Cycling #############################################

function dualcycling(; verbose = true, verbose_ws = true, rule = "mp", instance = "0", warmstart = "grka", sortmethod = "kids", 
                    timelimit = 20, rins = -1, cuts = 2, tugs = 4)

    path_to_input = joinpath(@__DIR__, "instances/input/"*rule*"_layout"*instance*".xlsx")

    if warmstart == "grka"
        filename = string("original_", rule, instance, "_", warmstart, "_", sortmethod)
    else
        filename = string("original_", rule, instance, "_", warmstart)
    end
...
#################################### Warm Start ##########################################    
    ...
    if warmstart == "single"
        include("1. Original/original single.jl")
        ws_objective = single_warmstart(path_to_input, filename, verbose_ws, upper_bound, tugs, T, S, P_s, P_q, x, y, wqq, wqv, wvq, u)
    end

    if warmstart == "grka"
        include("1. Original/original grka.jl")
        ws_objective = grka_warmstart(path_to_input, filename, verbose_ws, sortmethod, upper_bound, tugs, T, S, Q, P_s, P_q, x, y, wqq, wqv, wvq, u)
    end
...
#################################### Optimize ##########################################    
    ...
    save_results(model, rule , instance, warmstart, sortmethod, ws_objective, "original", "results.csv")
    if verbose == true
        save_loading_plan(filename, tugs, upper_bound, T, S, x, y, wvq, wqv, wqq)
    end
...
#=
function save_results(model, rule , instance, warmstart_type, warmstart_sort, warmstart_obj, model_type, save_file)
    best_objective = Int(JuMP.objective_value(model))
    gap = round(JuMP.relative_gap(model) * 100, digits= 2)
    bound  = Int(JuMP.objective_bound(model))
    solve_time = round(JuMP.solve_time(model), digits = 0)
    save_array = [model_type, instance, rule, warmstart_type, warmstart_sort, warmstart_obj, solve_time, best_objective, bound, gap]

    df = DataFrame(Model = [], Instance = [], Rule=[], Warmstart =[], Warmstart_sort = [], Warmstart_objective = [], Solve_time = [],
                   Best_objective = [], Best_bound = [], Gap = [])

    new_row = DataFrame([save_array], [:Model, :Instance, :Rule, :Warmstart, :Warmstart_sort, :Warmstart_objective, :Solve_time, :Best_objective, :Best_bound, :Gap])
    append!(df, new_row)
    #push!(df, save_array)
    CSV.write(save_file, df, delim = ',', append = true)
end
=#

function save_loading_plan(filename, tugs, upper_bound, T, S, x, y, wvq, wqv, wqq)
