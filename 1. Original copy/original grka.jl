function grka_warmstart(path_to_input, filename, verbose_ws, sortmethod, upper_bound, tugs, T, S, Q, P_s, P_q, x, y, wqq, wqv, wvq, u)
    ...
    assign_variables(tugs_array, x, y, wqq, wqv, wvq)
    set_start_value(u, t)
    objective = t

    ### PRINT THE RESULTS ###
    if verbose_ws == true
        print_warmstart(filename, tugs, upper_bound, T, S, x, y, wvq, wqv, wqq)
...
function get_complete_precedence(job_dict, S, P_s, P_q) 
    ...
    for (child, parent) in eachrow(P_q)  # P_q is the precedence matrix P (j, j')
        println("child = ", child, ",   parent = ", parent)
...
function sort_kids(DAG) # sort by number of descendants/kids
    ...
    #job_precedence_kids = sort(collect(job_precedence_kids), by=x->x[2], rev = true)
...
function assign_variables(tugs_array, x, y, wqq, wqv, wvq)
    for t in 1:length(tugs_array[1])
        wqq_tugs = 0
        wqv_tugs = 0
        wvq_tugs = 0
        for tug in tugs_array
        ...
        ############ LOADING #########
            ...
            if length(number_of_load) > 0
                for pq_done in number_of_load
                    ...
                    #has_path(DAG, precedence_value, pq_done) ? (continue) : nothing
                    has_path(DAG, precedence_value, pq_done) ? (@goto label1) : nothing
                end
            end
...
function print_warmstart(filename, tugs, upper_bound, T, S, x, y, wvq, wqv, wqq)
